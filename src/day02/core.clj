...
(defn split-into-rows
  {:test #(do
            (is (= (split-into-rows "4\n4") (list "4" "4")))
            (is (= (split-into-rows "5\t4\n5\t4") (list "5\t4" "5\t4")))
            (is (= (split-into-rows "5\t9\t2\t8\n9\t4\t7\t3\n3\t8\t6\t5") (list "5\t9\t2\t8" "9\t4\t7\t3" "3\t8\t6\t5")))
            )}
  [table]
  (split (trim table) #"\n"))
...
(defn split-into-columns
  {:test #(do
            (is (= (split-into-columns "7\t5\t3") (list 7 5 3)))
            (is (= (split-into-columns "4\t4\t\t5 ") (list 4 4 5)))
            (is (= (split-into-columns "4\t4 ") (list 4 4)))
            )}
  [row]
  (map parse-int (split (trim row) #"\t+")))

(defn get-min-and-max-diff
  {:test #(do
            (is (= (get-min-and-max-diff (list 5 1 9 5)) 8))
            )}
  [clean-row]
...
(defn pair-fraction
  {:test #(do
            (is (= (pair-fraction [1 3]) 3))
            (is (= (pair-fraction [3 1]) 3))
            (is (= (pair-fraction [100 20]) 5))
            (is (= (pair-fraction [20 100]) 5))
            )}
  [pair]
...
(defn valid-fraction?
  {:test #(do
            (is(= (valid-fraction? [1 1]) false))
            (is(= (valid-fraction? [1 3]) false))
            (is(= (valid-fraction? [4 3]) false))
            )}
  [pair]
...
(defn sum-of-rows
  {:test #(do
            (is (= (sum-of-rows [5 9 2 8]) 4))
            (is (= (sum-of-rows [9 4 7 3]) 3))
            (is (= (sum-of-rows [3 8 6 5]) 2))
            )}
  [clean-row]
